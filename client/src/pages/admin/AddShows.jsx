...
import { toast } from 'react-toastify' // Recommended for error feedback
...
const AddShows = () => {
  ...
  // FIX: Fetch from your Backend instead of dummyShowsData
  const fetchNowPlayingMovies = async () => {
    try {
      const { data } = await axios.get('http://localhost:3000/api/show/now-playing');
      if (data.success) {
        setNowPlayingMovies(data.movies); // Now this will contain Avatar, etc.
      }
    } catch (error) {
      ...
      <div className='overflow-x-auto pb-4'>
        <div className='group flex flex-wrap gap-4 mt-4 w-max'>
          {nowPlayingMovies.map((movie) => (
            // FIX: Changed movie.id to movie._id to match your backend mapping
            <div key={movie._id} 
                 className={`relative max-w-40 cursor-pointer group-hover:opacity-40 hover:opacity-100 hover:-translate-y-1 transition duration-300`} 
              ...
              <div className='relative rounded-lg overflow-hidden'>
                ...
                <div className='text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0'>
                  ...
                  <p className='text-gray-300'>Live Data</p>
                </div>
              </div>
